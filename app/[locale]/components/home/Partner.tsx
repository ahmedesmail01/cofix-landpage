...
const ValueCard = ({ valueKey }: { valueKey: string }) => {
  const t = useTranslations("Partner.values");
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm">
      ...
      <h3 className="text-lg font-semibold mb-2">{t(`${valueKey}.title`)}</h3>
      <p className="text-gray-600 text-sm">{t(`${valueKey}.description`)}</p>
    </div>
  );
};
